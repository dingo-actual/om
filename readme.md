...
If the final value dimension in the sequence differs from the first value dimension, the model performs a projection from the final value dimension to a user-defined dimension (with default `dim_input // num_heads`). This is done to reduce the number of parameters needed when projecting from the output dimension of the memory operation back to the input dimension.
